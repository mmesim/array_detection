function [saved_wvf]=my_output(header,detections, y_proc,ratio,thres,out_win)
...
%----------------------------------------------------------
if detections(k,1)-out_win >0 && detections(k,1)+out_win <length(y_proc(:,1))*header(1).DELTA
% Plot waveforms -- go through all available waveromfs
f1=figure('visible','off');
filename1=sprintf('waveforms_%03d.png',k);
for i=1:length(y_proc(1,:))
temp=y_proc(floor((detections(k,1)-out_win)./header(1).DELTA):round((detections(k,1)+out_win)./header(1).DELTA),i);
...
temp2=ratio(floor((detections(k,1)-out_win)./header(1).DELTA):round((detections(k,1)+out_win)./header(1).DELTA));
